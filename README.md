...
- [ ] **Custom Popularity Filtering**: Add configuration controls to the setup screen that let users filter game rounds using a tighter popularity threshold than what is stored in the database. For example, allowing users to restrict rounds dynamically to Top 10 peak hits (peak_position <= 10) or long-standing chart hits (weeks_on_chart >= 25).
