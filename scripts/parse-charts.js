...
const BILLBOARD_ALL_URL = 'https://raw.githubusercontent.com/mhollingshead/billboard-hot-100/main/all.json';
...
async function run() {
  console.log(`Fetching all Billboard Hot 100 history from: ${BILLBOARD_ALL_URL}...`);
  ...
  try {
    ...
    if (Array.isArray(allCharts)) {
      ...
      chartsIterable = Object.entries(allCharts).map(([date, data]) => ({ date, data }));
    }
    ...
    for (const chart of chartsIterable) {
      ...
      for (const entry of data) {
        ...
        const peakPosition = entry.peak_position !== undefined ? parseInt(entry.peak_position, 10) : parseInt(entry.this_week, 10);
        const weeksOnChart = entry.weeks_on_chart !== undefined ? parseInt(entry.weeks_on_chart, 10) : 1;
        
        const key = getSongKey(title, artist);
        const existing = songMap.get(key);
        
        if (existing) {
          existing.peak_position = Math.min(existing.peak_position, peakPosition);
          existing.weeks_on_chart = Math.max(existing.weeks_on_chart, weeksOnChart);
    ...
    for (const song of songMap.values()) {
      if (song.peak_position <= PEAK_LIMIT || song.weeks_on_chart >= WEEKS_LIMIT) {
        filteredSongs.push(song);
      }
    }
    
    // Sort by last chart date descending
    filteredSongs.sort((a, b) => b.last_chart_date.localeCompare(a.last_chart_date));
    ...
    console.log(`Successfully wrote ${filteredSongs.length} popular songs to ${OUTPUT_FILE}`);
